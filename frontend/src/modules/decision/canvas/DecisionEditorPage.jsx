  ...
  const renderConditionGroup = (branch, label, priority) => {
    ...
    <div className="selector-branch-card">
      ...
      {conditionRows[branch].map((row, rowIndex) => {
        ...
        const selectedOutsideOutput = selectedOutsideDecision?.outputs.find((output) => output.name === conditionLeftPicker?.outsideOutput)
                              ...
                              <div className="outside-temporary">
                                ...
                                <div className="outside-temporary-path">
                                  <label>
                                    ...
                                    <select
                                      ...
                                      onChange={(event) => setConditionLeftPicker((current) => ({
                                        ...
                                        outsidePolicyVersion: '',
                                        outsideDecision: '',
                                        outsideDecisionVersion: '',
                                        outsideOutput: '',
                                      }))}
                                    >
                                  ...
                                  <label>
                                    ...
                                    <select
                                      aria-label="Outside Temporary Policy Version"
                                      disabled={!selectedOutsidePolicy}
                                      value={conditionLeftPicker?.outsidePolicyVersion || ''}
                                      onChange={(event) => setConditionLeftPicker((current) => ({
                                        ...current,
                                        outsidePolicyVersion: event.target.value,
                                        outsideDecision: '',
                                        outsideDecisionVersion: '',
                                        outsideOutput: '',
                                      }))}
                                    >
                                      ...
                                      {(selectedOutsidePolicy?.versions || []).map((version) => (
                                ...
                                <div className="outside-temporary-path">
                                  <label>
                                    ...
                                    <select
                                      ...
                                      disabled={!conditionLeftPicker?.outsidePolicyVersion}
                                  ...
                                  <label>
                                    ...
                                    <select
                                      aria-label="Outside Temporary Decision Version"
                                      disabled={!selectedOutsideDecision}
                                      value={conditionLeftPicker?.outsideDecisionVersion || ''}
                                      onChange={(event) => setConditionLeftPicker((current) => ({
                                        ...current,
                                        outsideDecisionVersion: event.target.value,
                                        outsideOutput: '',
                                      }))}
                                    >
                                      <option value="">Select Version</option>
                                      {(selectedOutsideDecision?.versions || []).map((version) => (
                                        <option key={version} value={version}>{version}</option>
                                      ))}
                                    </select>
                                  </label>
                                </div>
                                <label className="outside-temporary-output">
                                  <span>Output</span>
                                  <select
                                    aria-label="Outside Temporary Output"
                                    disabled={!conditionLeftPicker?.outsideDecisionVersion}
                                    value={conditionLeftPicker?.outsideOutput || ''}
                                    onChange={(event) => setConditionLeftPicker((current) => ({
                                      ...current,
                                      outsideOutput: event.target.value,
                                    }))}
                                  >
                                ...
                                {selectedOutsideOutput && (
                                  <div className="outside-temporary-selected">
                                    <i className="temporary">T</i>
                                    <span>
                                      <b>t.{selectedOutsideOutput.name}</b>
                                      <small>
                                        {selectedOutsidePolicy.label} {conditionLeftPicker.outsidePolicyVersion}
                                        {' · '}{selectedOutsideDecision.label} {conditionLeftPicker.outsideDecisionVersion}
                                      </small>
                                    </span>
                                    <em>{selectedOutsideOutput.type}</em>
                                    <button
                                      type="button"
                                      onClick={() => chooseLeftVariable(
                                        `Outside Temporary · ${selectedOutsidePolicy.label} ${conditionLeftPicker.outsidePolicyVersion} · ${selectedOutsideDecision.label} ${conditionLeftPicker.outsideDecisionVersion} · ${selectedOutsideOutput.name}`,
                                      )}
                                    >Use Variable</button>
                                  </div>
                                )}
                    ...
                    {conditionValuePicker?.branch === branch && conditionValuePicker?.rowId === row.id && (
                      <div className="condition-reference-picker">
                        ...
                        <div className="condition-source-modes">
                          <button onClick={() => { updateCondition(branch, row.id, 'rightMode', 'value'); setConditionValuePicker(null) }}><span>Direct value</span><code>Type</code></button>
                          <button onClick={() => { updateCondition(branch, row.id, 'rightMode', 'expression'); if (!row.expressionParts) updateCondition(branch, row.id, 'expressionParts', []); setConditionValuePicker(null) }}><span>Expression</span><code>ƒx</code></button>
                        </div>
                        <small>Feature</small>
                        {featureVariables.map((feature) => (
                          <button key={feature.name} onClick={() => { updateCondition(branch, row.id, 'rightMode', 'value'); updateCondition(branch, row.id, 'expression', `Feature · ${feature.name}`); setConditionValuePicker(null) }}>
                            <span>{feature.name}</span><code>{feature.type}</code>
                          </button>
                        ))}
                        <small>Custom</small>
                        {customConditionVariables.map((variable) => (
                          <button key={variable.name} onClick={() => { updateCondition(branch, row.id, 'rightMode', 'value'); updateCondition(branch, row.id, 'expression', `Custom · ${variable.name}`); setConditionValuePicker(null) }}>
                        ...
                        {upstreamNodes.map((node) => (
                          <div key={node.id}>
                            ...
                            {(node.outputs || []).map((output) => (
                              <button key={output} onClick={() => { updateCondition(branch, row.id, 'rightMode', 'value'); updateCondition(branch, row.id, 'expression', `${node.label} · ${output}`); setConditionValuePicker(null) }}>
